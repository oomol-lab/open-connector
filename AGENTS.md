...
- Proxy: `defineProviderProxy` for every proxy whose request construction is the shared default. Hand-write `export const proxy: ProviderProxyExecutor = async ...` only when the proxy signs requests, rewrites the body, needs a non-JSON default content type, a custom response byte cap, or a bespoke error mapper, and name that reason in the pull request.
- AWS SigV4: `src/core/aws-sigv4.ts` is the only SigV4 implementation (`signAwsSigV4Request`, `createAwsSigV4PresignedUrl`, `sha256Hex`, `buildCanonicalHeaders`, `canonicalizeSearchParams`, `encodeRfc3986`, `encodeS3ObjectKey`). Do not add a second one; core already matches the AWS SDK header order and whitespace rules.
...
`src/providers/provider-source-guards.clone-baseline.test.ts` counts these clone classes across `src/providers` and compares the counts with `src/providers/clone-baseline.json`. The counts may only move down: removing a copy means lowering the matching entry in the same change, and a new copy fails the suite until it is removed or the entry is deliberately raised.
