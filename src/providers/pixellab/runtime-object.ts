...
export const pixellabObjectActionHandlers: ProviderActionHandlerSubset<"pixellab", PixellabObjectHandler> = {
      ...
      compactObject({
        ...
        item_descriptions:
          input.itemDescriptions === undefined
            ? undefined
            : stringArray(input.itemDescriptions, "itemDescriptions", providerInputError),
      }),
      context,
    );
  ...
  async start_animate_object(input, context) {
    ...
    if (!Array.isArray(record.submissions)) {
      throw providerResponseError("PixelLab object animation response is missing submissions.");
    }
    return compactObject({
      animationGroupId: requiredString(record.animation_group_id, "PixelLab animation_group_id", providerResponseError),
      mode: requiredString(record.mode, "PixelLab mode", providerResponseError),
      frameCount: responseInteger(record.frame_count, "PixelLab frame_count"),
      displayName: optionalString(record.display_name),
      description: requiredString(record.description, "PixelLab description", providerResponseError),
      objectId: requiredString(record.object_id, "PixelLab object_id", providerResponseError),
  ...
  async list_objects(input, context) {
    ...
    if (!Array.isArray(record.objects)) {
      throw providerResponseError("PixelLab object list is missing objects.");
    }
    return compactObject({
...
function normalizeDirectionSubmission(value: unknown, fieldName: string): Record<string, unknown> {
  ...
  return compactObject({
    direction: requiredString(record.direction, `${fieldName}.direction`, providerResponseError),
    status: requiredString(record.status, `${fieldName}.status`, providerResponseError),
...
function responseInteger(value: unknown, fieldName: string): number {
  ...
  if (number === undefined) throw providerResponseError(`${fieldName} must be an integer.`);
  return number;
}
