...
> = {
  get_ticker: (input, context) =>
    request("/api/1/ticker", { pair: requiredInputString(input.pair, "pair") }, undefined, context, "execute"),
  ...
  list_recent_trades: (input, context) =>
    request(
      "/api/1/trades",
      compactObject({ pair: requiredInputString(input.pair, "pair"), since: integerString(input.since) }),
      undefined,
      context,
      "execute",
    ),
  get_top_order_book: (input, context) =>
    request("/api/1/orderbook_top", { pair: requiredInputString(input.pair, "pair") }, undefined, context, "execute"),
  ...
  get_order: (input, context) =>
    request(
      `/api/1/orders/${encodeURIComponent(requiredInputString(input.id, "id"))}`,
      undefined,
      undefined,
      context,
      "execute",
...
function strings(value: unknown): string[] | undefined {
  ...
  return value.map((item) => requiredInputString(item, "array item"));
}
